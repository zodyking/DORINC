...
export const HELP_ANSWERS: HelpAnswer[] = [
  ...
  { keys: ['finalize', 'send', 'lock'], text: '<b>Finalize & send</b> locks totals server-side, generates the official PDF, and queues email + portal delivery. After finalize, line totals cannot be edited without creating a revision.' },
  ...
  { keys: ['payment', 'record', 'balance'], text: 'Open the invoice detail page and click <b>Record payment</b>. Enter amount, method, and date. The server validates against the open balance — overpayments are rejected.' },
  { keys: ['pdf', 'template'], text: 'Official PDFs are rendered server-side using your active template. Customize layout in <b>Template Designer</b>. Users download or email PDFs — they are never generated in the browser.' },
  ...
  { keys: ['service log', 'upload', 'review'], text: 'Mechanics upload logs from <b>Service Logs</b>. Files store in PostgreSQL bytea. Accountants review the queue, optionally approve AI-extracted line items, then create an invoice from the log.' },
  ...
  { keys: ['role', 'permission', 'accountant', 'mechanic', 'admin'], text: 'Account types: <b>Super Admin</b> (full system), <b>Accountant</b> (billing + approvals), <b>Mechanic</b> (vehicles + logs), <b>Customer</b> (portal only), <b>Viewer</b> (read-only). Permissions are enforced server-side.' },
  ...
  { keys: ['overdue', 'reminder'], text: 'An invoice becomes <b>overdue</b> when past its due date with an open balance. Use <b>Send</b> on the invoice detail page to queue delivery through SMTP.' },
  { keys: ['backup', 'restore'], text: 'Encrypted backups can be run manually from <b>Control Panel → Backup</b>. Configure schedule and Google Drive in later phases.' },
]
