...
export const APP_KNOWLEDGE_DOCS: AppKnowledgeDoc[] = [
  ...
  },
  {
    ...
    summary: 'Susan answers how-to questions in the in-app help widget and via Quo SMS using app knowledge tools.',
    body: [
      ...
      'Susan explains the product and can read invoices, service logs, customers, vehicles, catalog, AR aging, and revenue via tools when the staffer has permission.',
      'Susan does not mutate invoices/customers/data — lookups are read-only.',
      'Tools: get_app_knowledge, lookup_invoice, lookup_service_log, lookup_customer, lookup_vehicle, rank_customers, ar_aging, revenue_summary, search_catalog (permission-filtered).',
      'Invoice lookup: INV-000713 / invoice 713; unpaid|overdue|stats for KPIs; sort oldest|newest|amount_high for ranked lists with dates.',
      'rank_customers: top paying (lifetime_billed), open balance, amount paid, invoice count. ar_aging / revenue_summary for AR and collections.',
      'SL lookup: SL-0713 or review queue. Vehicles: bus/unit, VIN, plate.',
  ...
  {
    id: 'training-announcements',
    title: 'Training and announcements',
    area: 'training',
    pageKeys: ['default'],
    tags: ['training', 'announcements', 'required'],
    summary: 'Assigned training modules and required announcements for staff.',
    body: [
      'Training: /training and lesson pages; admins assign modules to users.',
      'Announcements: admin create/list; required announcements can gate staff until acknowledged.',
    ].join('\n'),
  },
  {
    id: 'staples-print',
