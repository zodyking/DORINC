...
/** @deprecated CSS object-fit handles fit; kept for tests that assert stage sizing math. */
export function computeImageFitScale(
  ...
  return Math.min(MAX_SCALE, Math.min(scaleW, scaleH))
}
...
/**
 * Pan + zoom for an image inside a container.
 * 100% zoom = image fits the stage via CSS; scale transforms zoom in/out from there.
 */
export function useImageZoomPan(containerRef: Ref<HTMLElement | null>) {
  ...
  const canPan = computed(() => scale.value > 1.01 || Math.abs(panX.value) > 1 || Math.abs(panY.value) > 1)
