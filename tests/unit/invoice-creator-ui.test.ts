...
describe('invoice-creator-ui helpers (P1-23)', () => {
  ...
  it('labels wizard steps', () => {
    expect(wizardStateLabel(1)).toBe('Step 1 of 6 — Customer')
    expect(wizardStateLabel(3)).toBe('Step 3 of 6 — Service log')
    expect(wizardStateLabel(4)).toBe('Step 4 of 6 — Dates & terms')
    expect(wizardStateLabel(6)).toBe('Step 6 of 6 — Review')
  })
  ...
  it('validates draft lines and step progression', () => {
    ...
    expect(canProceedWizardStep(4, { customerId: 'c1', vehicleId: 'v1', lines: [] })).toBe(true)
    expect(canProceedWizardStep(5, { customerId: 'c1', vehicleId: 'v1', lines: [line] })).toBe(true)
  })
