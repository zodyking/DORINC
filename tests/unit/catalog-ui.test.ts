...
describe('catalog-ui helpers (P1-19)', () => {
  ...
  it('normalizes legacy service rows to labor', () => {
    expect(normalizeCatalogItemType('service')).toBe('labor')
    expect(catalogTypeLabel('service')).toBe('Labor')
  })
  ...
  it('picks sensible default units per type', () => {
    ...
    expect(defaultUomForCatalogType('labor')).toBe('hr')
