...
describe('customer-email-staff-notify worker helper', () => {
  ...
  it('builds staff alert email with sign-in and reply instructions', () => {
    ...
    expect(mail.html).toContain('Fleet Co Sent A Message')
    expect(mail.html).toContain('Please call me back about the invoice.')
    expect(mail.html).toContain('border-left:3px solid #2563eb')
    expect(mail.html).toContain('display:block;width:100%')
    expect(mail.html).not.toContain('Notified')
    expect(mail.html).not.toContain('Customer message')
  })
})
