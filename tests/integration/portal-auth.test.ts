...
describe('P2-03 portal customer login', () => {
  it('logs in a portal user when portal is enabled', async () => {
    const result = await login(db, portalUserA.email, password)
  ...
  it('blocks login when portal is disabled', async () => {
    ...
    await expect(login(db, portalUserA.email, password)).rejects.toThrow('PORTAL_DISABLED')
  ...
  it('blocks login when customer link is missing', async () => {
    ...
    await expect(login(db, portalUserA.email, password)).rejects.toThrow('PORTAL_NOT_LINKED')
  ...
  it('blocks login when temporary password expired', async () => {
    ...
    await expect(login(db, portalUserA.email, password)).rejects.toThrow('TEMP_PASSWORD_EXPIRED')
    await db.update(users)
