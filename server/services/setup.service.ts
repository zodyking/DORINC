...
const BOOTSTRAP_LOCK_KEY = 0x44525042 // 'DRPB'
...
export async function bootstrapSuperAdmin(
  ...
  return db.transaction(async (tx) => {
    // Serialize concurrent bootstrap attempts on fresh installs (P4-07).
    await tx.execute(sql`SELECT pg_advisory_xact_lock(${BOOTSTRAP_LOCK_KEY})`)

    if (await isBootstrapped(tx)) throw new BootstrapLockedError()

    const [typeRow] = await tx.select().from(accountTypes).where(eq(accountTypes.key, 'super_admin'))
