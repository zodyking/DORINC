...
export default defineEventHandler(async (event) => {
  ...
  try {
    const { estimate, before } = await customerApproveEstimate(
      useDb(),
      id,
      user.customerId,
      user.id,
    ...
    await writeAudit(event, {
      entityType: 'estimate',
      entityId: id,
      action: 'estimates.customer_approve',
      beforeData: { status: before.status },
      afterData: { status: estimate.status, customerResponseNotes: estimate.customerResponseNotes },
      changedFields: ['status', 'customerApprovedAt', 'customerResponseNotes'],
    ...
    if (err instanceof EstimatesServiceError) {
      ...
      if (err.code === 'INVALID_TRANSITION') {
        throw apiError(event, 'CONFLICT', 'This estimate cannot be approved in its current state')
      }
    }
    throw err
  }
