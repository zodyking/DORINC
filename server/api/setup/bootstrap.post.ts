...
import { assertSetupRequirements } from '../../services/app-config.service'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    await assertSetupRequirements(db)
  }
  catch (err) {
    const code = (err as Error).message.replace('SETUP_INCOMPLETE: ', '')
    throw apiError(event, 'VALIDATION_ERROR', `Complete setup step "${code}" before creating the Super Admin`)
  }
