...
import { CustomersServiceError, restoreCustomer } from '../../../services/customers.service'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const customer = await restoreCustomer(useDb(), id)
    ...
    if (err instanceof CustomersServiceError) {
      ...
      if (err.code === 'NOT_ARCHIVED') throw apiError(event, 'CONFLICT', 'Customer is not archived')
    }
    throw err
  }
})
